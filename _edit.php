<?php
require_once 'vendor/autoload.php';
    $user = new \app\back\Login();
    $consult = $user->edit($_GET['id']);

    foreach($consult as $pessoa):
    echo
    '
    <!DOCTYPE html>
    <html lang="pt-br">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="app/front/css/register.css">
        <title> Editar Dados </title>
    </head>
    
    <body>
        <section>
            <div class="formbox">
                <form action="_update.php" method="post">
                    <h2> REGISTRAR </h2>
                    <div class="container">
                        <div class="personal">
                            <input type="hidden" name="id" value="'.$pessoa['id'].'">

                            <div class="inputbox">
                                <label for=""> Nome: </label>
                                <input type="text" name="nome" id="nome" value="'.$pessoa['nome'].'">
                            </div>
                            
                            <div class="inputbox">
                                <label for=""> Email: </label>
                                <input type="text" name="email" id="email" value="'.$pessoa['email'].'">
                            </div>
                        </div>
                        
                        <div class="user">    
                            <div class="inputbox">
                                <label for=""> Usuário: </label>    
                                <input type="text" name="user" id="user" value="'.$pessoa['user'].'">
                            </div>
                        
                            <div class="inputbox">
                                <label for=""> Nova Senha: </label>
                                <input type="password" name="password">
                            </div>
                            
                            <div class="inputbox">
                                <label for=""> Confirmar Senha: </label>
                                <input type="password" name="password_confirm">
                            </div>
                        </div>
                    </div>
                        <button> ENVIAR </button> <br> <br>
                        <a href="consultar.php"> Voltar </a>
                </form>
            </div>
        </section>
        </body>
    </html>
    ';
    endforeach;
?>